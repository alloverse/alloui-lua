...
local textColor = {0,0,0,1.0}
local inactiveColor = {0.6, 0.6, 0.6, 1.0}
local activeColor =   {0.9, 0.9, 0.9, 1.0}
...
function TabView:switchToTabIndex(index)
    ...
    if newTab then
        ...
        newTab.button:setColor(activeColor)
    end
    if oldTab then 
        oldTab.view:removeFromSuperview()
        oldTab.button:setColor(inactiveColor)
    end
end
